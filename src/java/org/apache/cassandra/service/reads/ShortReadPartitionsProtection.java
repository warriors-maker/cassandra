...
public class ShortReadPartitionsProtection extends Transformation<UnfilteredRowIterator> implements MorePartitions<UnfilteredPartitionIterator>
{
    ...
    private UnfilteredPartitionIterator executeReadCommand(ReadCommand cmd)
    {
        ...
        DataResolver resolver = new DataResolver(keyspace, cmd, ConsistencyLevel.TREAS, 1, queryStartNanoTime, NoopReadRepair.instance);
        ReadCallback handler = new ReadCallback(resolver, ConsistencyLevel.TREAS, cmd, Collections.singletonList(source), queryStartNanoTime, NoopReadRepair.instance);

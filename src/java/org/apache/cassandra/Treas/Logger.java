...
public class Logger
{
    ...
    public void writeMutateStats(long num, long start, long end, String value, String key) {
        synchronized (obj1) {
            int index = TreasConfig.getAddressMap().get(FBUtilities.getJustLocalAddress().toString().substring(1));
            String name = absPath + "treasMuateStats" + index + ".txt";
            ...
            try {
                printWriter.write(num +  " " + start + ' ' + end + ' ');
    ...
    public void writeReadStats(long num, long start, long end, String value, String key) {
        synchronized (obj2) {
            int index = TreasConfig.getAddressMap().get(FBUtilities.getJustLocalAddress().toString().substring(1));
            String name = absPath + "treasReadStats" + index + ".txt";
            FileWriter writer = null;
            try
            {
                initFile(name);
